...
class FlaskSQLAlchemyInstaller(FlaskExtInstaller):
    ...
    def install():
        ...
        if not os.path.exists("src/models"):
            ...
            f = open("src/models/__init__.py", "w+")
            f.close()
        ...
        with open("src/config.py", "r+") as f:
            ...
            while i < len(lines):
                if lines[i] == "# --flask_boot_mark base_config--":
                    lines.insert(i, "\tSQLALCHEMY_DATABASE_URI=os.environ.get(\"DATABASE_URL\")")
        ...
        with open("src/__init__.py", "r+") as f:
            ...
            while i < len(lines):
                if lines[i] == "from flask_sqlalchemy import SQLAlchemy":
                    ...
                    del lines[i]
                    i -= 1 # keep cursor in same position
                elif lines[i] == "\t\tdb.init_app(app)" or lines[i] == "\t\tdb.create_all()":
                    del lines[i]
                    i -= 1 # keep cursor in same position
                i += 1
            f.seek(0)
